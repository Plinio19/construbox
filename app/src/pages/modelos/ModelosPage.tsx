...
export default function ModelosPage() {
  ...
  useEffect(() => { fetchModelos(); fetchCatalogo(); fetchEtapas(); }, []);
  ...
  async function moverEtapa(id: string, direcao: 'up' | 'down') {
    const etapas = [...(modeloAtual?.etapas || [])];
    const idx = etapas.findIndex(e => e.id === id);
    if (idx === -1) return;
    if (direcao === 'up' && idx === 0) return;
    if (direcao === 'down' && idx === etapas.length - 1) return;
    const swap = direcao === 'up' ? idx - 1 : idx + 1;
    [etapas[idx], etapas[swap]] = [etapas[swap], etapas[idx]];
    await salvarEtapasModelo(etapas);
  }
                      ...
                      <Col>
                        <Space size={4} onClick={e => e.stopPropagation()}>
                          ...
                          <Tooltip title="Mover para cima">
                            <Button size="small" icon={<ArrowUpOutlined />} disabled={i === 0} onClick={() => moverEtapa(et.id, 'up')} />
                          </Tooltip>
                          <Tooltip title="Mover para baixo">
                            <Button size="small" icon={<ArrowDownOutlined />} disabled={i === etapas.length - 1} onClick={() => moverEtapa(et.id, 'down')} />
                          </Tooltip>
